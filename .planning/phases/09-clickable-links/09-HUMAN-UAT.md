...
updated: 2026-06-21T02:05:00Z
---

## Current Test

[human testing complete — 1 gap found, routing to gap closure]
...
result: ISSUE — clickability and click-region (tally outside) both work, BUT the link target is wrong. It currently opens `https://api.weather.gov/alerts/active?zone=<UGC>`, which renders raw CAP **JSON** in the browser, not a human-readable page. Should open the human-readable NWS hazards page. See Gaps → GAP-09-A.
...
result: skipped (not tested). Fix retained as GAP-09-B (low-risk conservative hardening).
...
truth: Clicking a weather alert opens `https://api.weather.gov/alerts/active?zone=<UGC>` — raw CAP JSON — instead of a readable NWS hazards page.
...
truth: `_osc8_enabled` returns True for any non-empty `VTE_VERSION`, but VTE only added OSC 8 in 0.50 (`VTE_VERSION >= 5000`). Old VTE terminals on `auto` could see raw escapes.
